class Product < ApplicationRecord
  ...
  def self.searchs searchs
    if searchs
      if searchs[:pricefrom].present? && searchs[:priceto].present?
        results = self.search_price(searchs[:pricefrom], searchs[:priceto])
      elsif searchs[:priceto].present?
        result = self.search_to(searchs[:priceto])
      elsif searchs[:pricefrom].present?
        result = self.search_from(searchs[:pricefrom])
      elsif result.present?
        results = results.select_cate_product(searchs[:sltCategory]) if searchs[:sltCategory].present?
      elsif result.blank?
        results= self.select_cate_product(searchs[:sltCategory]) if searchs[:sltCategory].present?
      end
    else
      where(nil)
    end
